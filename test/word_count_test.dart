...
void simpleTests() {
  test('count one word', () {
    final Map result = wordCount.countWords('word');
    expect(result, equals(<String, int>{'word': 1}));
  }, skip: false);

  test('count one of each word', () {
    final Map result = wordCount.countWords('one of each');
...
void ignoreSpecialCharacters() {
  test('handles cramped lists', () {
    final Map result = wordCount.countWords('one,two,three');
    expect(result, equals(<String, int>{'one': 1, 'two': 1, 'three': 1}));
  }, skip: true);

  test('handles expanded lists', () {
    final Map result = wordCount.countWords('one,\ntwo,\nthree');
    expect(result, equals(<String, int>{'one': 1, 'two': 1, 'three': 1}));
  }, skip: true);
...
void notJustWords() {
  test('include numbers', () {
    final Map result = wordCount.countWords('testing, 1, 2 testing');
...
void edgeCases() {
  test('normalize case', () {
    final Map result = wordCount.countWords('go Go GO Stop stop');
